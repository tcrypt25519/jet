...
impl<'ctx> SymbolicStack<'ctx> {
    ...
    pub(crate) fn push_word_with_known_u64(&mut self, v: IntValue<'ctx>, known_u64: Option<u64>) {
        self.slots.push(StackValue::Word { value: v, known_u64 });
    }

    pub(crate) fn pop_word(&mut self) -> Result<IntValue<'ctx>, Error> {
        let value = self.slots.pop().ok_or_else(|| Error::invariant_violation("symbolic stack underflow"))?;
        match value {
    ...
    pub(crate) fn swap(&mut self, index_1_based: u8) -> Result<(), Error> {
        ...
        let top_idx = len.checked_sub(1).ok_or_else(|| Error::invariant_violation("symbolic stack underflow"))?;
        let swap_idx = len
