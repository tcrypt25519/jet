...
impl<'ctx> RuntimeBuilder<'ctx> {
    ...
    pub fn build(self) -> Module<'ctx> {
        ...
        self.module.add_global(self.types.ptr, None, crate::symbols::JIT_ENGINE);
    ...
    fn declare_external_builtins(&self) {
        ...
        self.module
            .add_function("jet.ops.exp", self.types.i8.fn_type(&[self.types.ptr.into(), self.types.ptr.into()], false), None);

        self.module.add_function(
            "jet.ops.addmod",
            self.types
                .i8
                .fn_type(&[self.types.ptr.into(), self.types.ptr.into(), self.types.ptr.into(), self.types.ptr.into()], false),
            None,
        );
        ...
        self.module.add_function(
            "jet.mem.expand",
            self.types.i8.fn_type(&[self.types.ptr.into(), self.types.i32.into(), self.types.i32.into()], false),
            None,
        );
    }
    ...
    fn build_stack_push_i256(&self) -> FunctionValue<'ctx> {
        let fn_type = self
            .context
            .bool_type()
            .fn_type(&[self.context.ptr_type(inkwell::AddressSpace::default()).into(), self.types.i256.into()], false);
        ...
        let stack_ptr_addr = self.builder.build_struct_gep(self.types.exec_ctx, ctx_ptr, 0, "stack.ptr.addr").unwrap();
        let stack_ptr = self.builder.build_load(self.types.i32, stack_ptr_addr, "stack.ptr").unwrap().into_int_value();
        let is_full = self
            .builder
            .build_int_compare(
                inkwell::IntPredicate::UGE,
                stack_ptr,
                self.types.i32.const_int(jet_ir::STACK_SIZE_WORDS as u64, false),
                "stack.is_full",
            )
            .unwrap();
        ...
        self.builder.build_conditional_branch(is_full, overflow_block, valid_block).unwrap();
        self.builder.position_at_end(overflow_block);
        self.builder.build_return(Some(&self.context.bool_type().const_zero())).unwrap();
        self.builder.position_at_end(valid_block);

        // Get address of stack[stack_ptr] (field 5 is stack array)
        let stack_field_ptr = self.builder.build_struct_gep(self.types.exec_ctx, ctx_ptr, 5, "stack.field").unwrap();

        // Index into the stack array
        let stack_top_addr = unsafe { self.builder.build_gep(self.types.i256, stack_field_ptr, &[stack_ptr], "stack.top.addr").unwrap() };

        // Store the value
        self.builder.build_store(stack_top_addr, value).unwrap();

        // Increment stack pointer
        let stack_ptr_next = self.builder.build_int_add(stack_ptr, self.types.i32.const_int(1, false), "stack.ptr.next").unwrap();
    ...
    fn build_stack_push_word(&self) -> FunctionValue<'ctx> {
        let fn_type = self
            .context
            .bool_type()
            .fn_type(&[self.context.ptr_type(inkwell::AddressSpace::default()).into(), self.types.ptr.into()], false);
        ...
        let stack_ptr_addr = self.builder.build_struct_gep(self.types.exec_ctx, ctx_ptr, 0, "stack.ptr.addr").unwrap();
        let stack_ptr = self.builder.build_load(self.types.i32, stack_ptr_addr, "stack.ptr").unwrap().into_int_value();
        let is_null = self.builder.build_is_null(word_ptr, "word.is_null").unwrap();
        let is_full = self
            .builder
            .build_int_compare(
                inkwell::IntPredicate::UGE,
            ...
            .unwrap();
        let invalid = self.builder.build_or(is_null, is_full, "stack.push.invalid").unwrap();
        let invalid_block = self.context.append_basic_block(function, "invalid");
        let valid_block = self.context.append_basic_block(function, "valid");
        self.builder.build_conditional_branch(invalid, invalid_block, valid_block).unwrap();
        self.builder.position_at_end(invalid_block);
        self.builder.build_return(Some(&self.context.bool_type().const_zero())).unwrap();
        self.builder.position_at_end(valid_block);
        let word_value = self.builder.build_load(self.types.i256, word_ptr, "word.value").unwrap().into_int_value();
    ...
    fn build_stack_pop(&self) -> FunctionValue<'ctx> {
        let fn_type = self.types.ptr.fn_type(&[self.context.ptr_type(inkwell::AddressSpace::default()).into()], false);
        ...
        let is_underflow = self
            .builder
            .build_int_compare(inkwell::IntPredicate::EQ, stack_ptr, self.types.i32.const_zero(), "is_underflow")
            .unwrap();

        let underflow_block = self.context.append_basic_block(function, "underflow");
        let valid_block = self.context.append_basic_block(function, "valid");

        self.builder.build_conditional_branch(is_underflow, underflow_block, valid_block).unwrap();
        ...
        let stack_ptr_prev = self.builder.build_int_sub(stack_ptr, self.types.i32.const_int(1, false), "stack.ptr.prev").unwrap();
        ...
        let stack_top_addr = unsafe {
            self.builder
                .build_gep(self.types.i256, stack_field_ptr, &[stack_ptr_prev], "stack.top.addr")
                .unwrap()
        };
    ...
    fn build_stack_peek(&self) -> FunctionValue<'ctx> {
        let fn_type = self
            .types
            .ptr
            .fn_type(&[self.context.ptr_type(inkwell::AddressSpace::default()).into(), self.types.i8.into()], false);
        ...
        let peek_idx_32 = self.builder.build_int_z_extend(peek_idx, self.types.i32, "peek.idx.32").unwrap();
        ...
        let is_underflow = self
            .builder
            .build_int_compare(inkwell::IntPredicate::UGE, peek_idx_32, stack_ptr, "peek.is_underflow")
            .unwrap();
        let underflow_block = self.context.append_basic_block(function, "underflow");
        let valid_block = self.context.append_basic_block(function, "valid");
        self.builder.build_conditional_branch(is_underflow, underflow_block, valid_block).unwrap();
        self.builder.position_at_end(underflow_block);
        self.builder.build_return(Some(&self.types.ptr.const_null())).unwrap();
        self.builder.position_at_end(valid_block);

        // Calculate index: stack_ptr - peek_idx - 1
        let idx_temp = self.builder.build_int_sub(stack_ptr, peek_idx_32, "idx.temp").unwrap();
        let idx = self.builder.build_int_sub(idx_temp, self.types.i32.const_int(1, false), "idx").unwrap();

        // Get address of stack[idx] (field 5 is stack array)
        let stack_field_ptr = self.builder.build_struct_gep(self.types.exec_ctx, ctx_ptr, 5, "stack.field").unwrap();

        let stack_elem_addr = unsafe { self.builder.build_gep(self.types.i256, stack_field_ptr, &[idx], "stack.elem.addr").unwrap() };
    ...
    fn build_stack_swap(&self) -> FunctionValue<'ctx> {
        ...
        let swap_idx_32 = self.builder.build_int_z_extend(swap_idx, self.types.i32, "swap.idx.32").unwrap();
        ...
        let required_depth = self
            .builder
            .build_int_add(swap_idx_32, self.types.i32.const_int(2, false), "swap.required_depth")
            .unwrap();
        let is_underflow = self
            .builder
            .build_int_compare(inkwell::IntPredicate::ULT, stack_ptr, required_depth, "swap.is_underflow")
            .unwrap();
        let underflow_block = self.context.append_basic_block(function, "underflow");
        let valid_block = self.context.append_basic_block(function, "valid");
        self.builder.build_conditional_branch(is_underflow, underflow_block, valid_block).unwrap();
        ...
        let top_idx = self.builder.build_int_sub(stack_ptr, self.types.i32.const_int(1, false), "top.idx").unwrap();

        // Calculate swap_with_idx: stack_ptr - 2 - swap_idx
        let temp = self.builder.build_int_sub(stack_ptr, self.types.i32.const_int(2, false), "temp").unwrap();
        let swap_with_idx = self.builder.build_int_sub(temp, swap_idx_32, "swap.with.idx").unwrap();

        // Get address of stack array (field 5)
        let stack_field_ptr = self.builder.build_struct_gep(self.types.exec_ctx, ctx_ptr, 5, "stack.field").unwrap();

        // Get address of top element
        let top_addr = unsafe { self.builder.build_gep(self.types.i256, stack_field_ptr, &[top_idx], "top.addr").unwrap() };

        // Get address of swap element
        let swap_addr = unsafe { self.builder.build_gep(self.types.i256, stack_field_ptr, &[swap_with_idx], "swap.addr").unwrap() };

        // Load both values
        let top_val = self.builder.build_load(self.types.i256, top_addr, "top.val").unwrap().into_int_value();
        let swap_val = self.builder.build_load(self.types.i256, swap_addr, "swap.val").unwrap().into_int_value();
    ...
    fn build_mem_load(&self) -> FunctionValue<'ctx> {
        ...
        let loc = self.builder.build_load(self.types.i32, loc_ptr, "loc").unwrap().into_int_value();

        // Get memory_ptr (field 6)
        let mem_ptr_addr = self.builder.build_struct_gep(self.types.exec_ctx, ctx_ptr, 6, "mem.ptr.addr").unwrap();
        let mem_ptr = self.builder.build_load(self.types.ptr, mem_ptr_addr, "mem.ptr").unwrap().into_pointer_value();

        // Calculate byte offset in memory
        let byte_ptr = unsafe { self.builder.build_gep(self.types.i8, mem_ptr, &[loc], "byte.ptr").unwrap() };
        ...
        let value = self.builder.build_load(self.types.i256, byte_ptr, "mem.value").unwrap().into_int_value();
    ...
    fn build_mem_store_word(&self) -> FunctionValue<'ctx> {
        ...
        let dest_ptr = unsafe { self.builder.build_gep(self.types.i8, mem_ptr, &[loc], "dest.ptr").unwrap() };

        // Copy 32 bytes from val_ptr to dest_ptr
        let word_size = self.types.i32.const_int(32, false);
        self.builder.build_memcpy(dest_ptr, 1, val_ptr, 1, word_size).unwrap();
    ...
    fn build_mem_store_byte(&self) -> FunctionValue<'ctx> {
        ...
        // Load byte value
        let byte_val = self.builder.build_load(self.types.i8, val_ptr, "byte.val").unwrap().into_int_value();
