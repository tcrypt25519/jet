...
0. `offset`: 2-byte offset in the [return data](./about.mdx) from the last executed sub [context](./about.mdx) to copy.

## Stack output

0. `data`: 32-byte value starting from the given offset of the [return data](./about.mdx). All bytes after the end of the calldata are set to 0.
